...
from py_compile import compile as py_compile_file  # For auto .pyc conversion
...
# Helper function to auto-convert .py to .pyc
def auto_convert_to_pyc(py_file):
    if not py_file.endswith(".py"):
        print("[-] Not a .py file, skipping .pyc conversion.")
        return
    pyc_file = py_file + "c"  # e.g., script.py -> script.pyc
    try:
        py_compile_file(py_file, pyc_file, doraise=True)
        print(f"[+] Successfully compiled: {py_file} → {pyc_file}")
    except Exception as e:
        print(f"[-] Failed to compile to .pyc: {e}")
...
def ahm():
    ...
    exec(a)
    # No file saved here, so no .pyc conversion
...
def decode_base64_file():
    ...
    auto_convert_to_pyc("Decode_base64.py")
...
def decode_base16_file():
    ...
    auto_convert_to_pyc("Decode_base16.py")
...
def decode_base32_file():
    ...
    auto_convert_to_pyc("Decode_base32.py")
...
def decode_base85_file():
    ...
    auto_convert_to_pyc("Decode_base85.py")
...
def decode_hex():
    ...
    auto_convert_to_pyc(decoded_filename)
