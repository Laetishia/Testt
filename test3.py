...
# NEW & IMPROVED: Detect obfuscation by pasting code directly
def detect_obfuscation():
    ...
    while True:
        try:
            line = input()
            if line == "" and len(lines) > 0 and lines[-1] == "":  # Two empty lines to finish
                break
            lines.append(line)
        except EOFError:
            break
    ...
    # 1. Marshal
    if 'marshal.loads' in content or 'marshal.dumps' in content:
        suggestions.append("➲ 1 or 2: Marshal obfuscation detected (high confidence)")
        confidence.append("High")
        if 'zlib.decompress' in content:
            suggestions.append("➲ 14 or 7: Combined with zlib compression")
        if 'base64' in content.lower():
            suggestions.append("➲ 4 or 7: Combined with base64 encoding")

    # 2. Lambda complex
    if 'lambda' in content and ('__import__' in content or 'exec((' in content or '_ = lambda' in content):
        suggestions.append("➲ 8: Lambda-based obfuscation (common in advanced payloads)")
        confidence.append("High")

    # 3. Emoji / Unicode heavy
    unicode_count = sum(1 for c in content if ord(c) > 127)
    if unicode_count > 50 or (len(content) > 100 and unicode_count / len(content) > 0.3):
        suggestions.append("➲ 9: Heavy emoji/unicode obfuscation")
        confidence.append("High")

    # 4. Pure Base encodings
    if 'base64.b64decode' in content and 'marshal' not in content:
        suggestions.append("➲ 4: Pure Base64 encoding")
    if 'base32' in content:
        suggestions.append("➲ 3: Base32")
    if 'base16' in content or 'hex' in content.lower():
        suggestions.append("➲ 5 or 10: Base16/Hex")
    if 'base85' in content:
        suggestions.append("➲ 6: Base85")

    # 5. Compression
    if 'zlib.decompress' in content and 'marshal' not in content:
        suggestions.append("➲ 14: Zlib compression")
    if 'gzip' in content.lower():
        suggestions.append("➲ 15: Gzip")
    if 'lzma' in content:
        suggestions.append("➲ 19 or 12: Lzma")

    # 6. Codecs
    if 'codecs.decode' in content or 'rot13' in content:
        suggestions.append("➲ 16: Codecs (rot13, etc.)")

    # 7. Binascii
    if 'binascii' in content:
        suggestions.append("➲ 13: Binascii")

    # 8. Simple exec with long string (common in marshal/base64)
    if content.startswith('exec(') or content.startswith('eval('):
        suggestions.append("➲ 1, 4, or 7: Likely wrapped exec with encoded payload")

    # 9. Looks like compiled pyc (binary data)
    if len(content) < 100 and any(ord(c) < 32 or ord(c) > 126 for c in content[:50]):
        suggestions.append("➲ 17 or 18: This looks like a compiled .pyc file (binary) – save as file first")

    # Final output
    if suggestions:
        print(a16 + "Recommended decoding options:\n")
        for i, sug in enumerate(suggestions, 1):
            print(a20 + f"   {i}. {sug}")
        print("\n" + a40 + "Go back to the main menu and choose one of the suggested numbers!")
        print(a7 + "\nTip: Higher confidence suggestions appear first.")
    else:
        print(a5 + "No common obfuscation pattern detected.")
        print(a11 + "It might be plain Python, very advanced obfuscation, or a binary .pyc file.")
...
def PY_97():
    ...
    print(a20 + ' ➲ 20. Cython    ')
    print(a20 + ' ➲ 21. Detect type by PASTING code (NO FILE NEEDED!)    ')
    print(a20 + ' ➲ 0. Exit    ')
    print(a16 + '⏔' * 28)
