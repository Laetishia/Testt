...
def detect_obfuscation():
    file_path = input("\x1b[1;31m[\x1b[1;31mPY_97\x1b[1;31m] \033[1;97m> - Enter File to detect : ")
    try:
    ...
    print("\n" + a40 + "Detecting obfuscation type..." + "\n")
    
    suggestions = []
    
    if 'marshal.loads' in content:
        suggestions.append("Likely Marshal obfuscated → Try option 1 or 2")
        if 'zlib.decompress' in content:
            suggestions.append("→ Also zlib → Try option 14 or 7")
        if 'base64' in content:
            suggestions.append("→ Also base64 → Try option 4 or 7")
    
    if 'lambda' in content and ('__import__' in content or '.__' in content):
        suggestions.append("Likely Lambda obfuscation → Try option 8")
    
    if any(ord(c) > 127 for c in content) and sum(ord(c) > 127 for c in content) > 30:
        suggestions.append("Many unicode/emoji characters → Try option 9 (Decode emoji)")
    
    if 'base64.b64decode' in content and 'marshal' not in content:
        suggestions.append("Pure Base64 → Try option 4")
    if 'base64.b32decode' in content:
        suggestions.append("Base32 → Try option 3")
    if 'base64.b16decode' in content:
        suggestions.append("Base16 → Try option 5")
    if 'base64.b85decode' in content:
        suggestions.append("Base85 → Try option 6")
    
    if 'zlib.decompress' in content and 'marshal' not in content:
        suggestions.append("Zlib compressed → Try option 14")
    if 'gzip' in content:
        suggestions.append("Gzip → Try option 15")
    if 'lzma' in content:
        suggestions.append("Lzma → Try option 19 or 12")
    
    if 'codecs.decode' in content:
        suggestions.append("Codecs obfuscation → Try option 16")
    
    if 'binascii' in content:
        suggestions.append("Binascii → Try option 13")
    
    if content.strip().startswith('exec(') or content.strip().startswith('print('):
        suggestions.append("Simple exec/lambda → Try option 8")
    
    # Rough pyc check (binary header)
    try:
        with open(file_path, 'rb') as f:
            header = f.read(16)
        if b'marshal' in header or header.startswith(b'\x03\xf3') or header.startswith(b'\xee\x0c\r\n'):
            suggestions.append("Compiled .pyc file → Try option 17 or 18 (simple pyc)")
    except:
        pass
    
    if suggestions:
        print(a16 + "Detected possible types:\n")
        for sug in suggestions:
            print("   " + a20 + "• " + sug)
        print("\n" + a40 + "Now go back and choose the correct option number!")
    else:
        print(a5 + "No known obfuscation detected. Might be plain Python or unknown method.")
...
def PY_97():
    ...
    print(a20 + ' ➲ 20. Cython    ')
    print(a20 + ' ➲ 21. Detect obfuscation type (NEW!)    ')  # NEW OPTION
    print(a20 + ' ➲ 0. Exit    ')
    print(a16 + '⏔' * 28)
